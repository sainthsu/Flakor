...
Vec4 Mat4::operator*( const Vec4& rhs ) const
{
 ...
 //伴随矩阵法求矩阵的逆
 //A-1 = A*/|A|
 //A* 为伴随矩阵，伴随矩阵de第i 行第j 列的元素是A关于第j 行第i 列的代数余子式
 //|A|为矩阵的A的行列式
 //
 //分块求逆法：
 //|A C|-1 = |A-1 -(A-1)×C×(B-1)|
...
Mat4 Mat4::Orthographic( float width, float height, float nearPlane, float farPlane )
{
  ...
  result.f_[12] = 0;
  result.f_[1] = 0;
  result.f_[5] = 1/ height;
  result.f_[9] = 0;
  result.f_[13] = 0;
  result.f_[2] = 0;
  result.f_[6] = 0;
  result.f_[10] = -2/(farPlane - nearPlane);
  result.f_[14] = (farPlane+nearPlane)/(nearPlane-farPlane);
  result.f_[3] = 0;
  result.f_[7] = 0;
  result.f_[11] = 0;
  result.f_[15] = 0;
